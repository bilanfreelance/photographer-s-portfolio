...
// Gallery (show more foto)

let galleryButton = document.querySelector('.gallery__button');
let galleryButtonVisit = document.querySelector('.gallery__button-visit');
let galleryColumns = document.querySelectorAll('.gallery__column_more');
if (galleryColumns.length > 0) {
	for (let index = 0; index < galleryColumns.length; index++) {
		const galleryColumn = galleryColumns[index];
		galleryButton.addEventListener("click", function (e) {
			e.preventDefault();
			galleryColumn.classList.remove('_viev');
			galleryButton.classList.add('_hide');
			galleryButtonVisit.classList.remove('_hide');
		});
		if (galleryButtonVisit) {
			galleryButtonVisit.addEventListener("click", function (e) {
				e.preventDefault();
				galleryColumn.classList.add('_viev');
				galleryButton.classList.remove('_hide');
				galleryButtonVisit.classList.add('_hide');
			});
		}
	}
}
...
let slider_foto = new Swiper('.slider__body', {
	autoplay: {
		delay: 500,
		disableOnInteraction: false,
	},
